...
qtl.loc <- rbind(c(10,25,0))
qtl.model <- "y~Q1"
# Heritability
h=0.08 # 8%
add.effect=sqrt(4*h/(1-2*h))
effect.vec <- c(add.effect)
...
n.cores=detectCores()-12
n.sim <- 1000
max.qtl <- 10
k_f <- 3
results <- mclapply(1:n.sim, stepwiseqtlcompare, mc.cores=n.cores, mc.preschedule=FALSE)
desc <- paste("QTL 1.42-8, ", n.sim," runs with max.qtl=",max.qtl, ", compare all methods, ",  k_f, "*main penalty below best")
filename <- "~/Research/QTL/result/ResultsStepwiseCompareMethods1main8percentv9.RData"
